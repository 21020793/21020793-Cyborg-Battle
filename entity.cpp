		...
		if (collisions.size() > 0){
			...
			while (!foundCollision){
				...
				if (distanceBetweenTwoRects(sampleBox, collisionBox) > boxTravelSize){
					...
					/* have to add these two line cuz sampleBox.xand y are interger while xMoveand ymove are float(which will automatically cut off
					   the decimal part when adding these two together)*/
