...
void Hero::slash()
{
	...
	{
		moving = false;
		frameTimer = 0; // decide when to take over animations
...
void Hero::updateAnimation()
{
	...
	// hero standing so change anim to idle 
	...
	{
		//if we are at the end of the anim
		if (currentFrame->frameNumber == currentAnim->getEndFrameNumber())
		{
			...
			{
				// was dead but somehow have more hp, get back up
				changeAnimation(HERO_STATE_MOVE, true);
			}
			else
			{
		...
		{
			// just move to the next frame in this animation
...
void Hero::updateDamages()
{
	if (active && hp > 0 && invincibleTimer <= 0)
	{
		for (auto entity = Entity::entities.begin(); entity != Entity::entities.end(); entity++)
		{
			...
			{
				// we know enemies are living entity so cast it to that
